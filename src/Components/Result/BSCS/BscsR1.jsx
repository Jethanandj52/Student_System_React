    ...
    <div className="bg-gray-100 p-4">
      {filteredData.length > 0 ? (
        filteredData.map((student, index) => {
          // Subject Marks
          const subjects = [
            { name: "Calculus", marks: student.calculus },
            { name: "Physics", marks: student.physics },
            { name: "Computing", marks: student.computing },
            { name: "English", marks: student.english },
            { name: "Pak Studies", marks: student.pakStudies },
            { name: "Programming", marks: student.programming },
          ];
          ...
          return (
            <div key={index} className="mb-6 p-4 bg-white shadow rounded">
              <div className='flex justify-between mb-4'>
                ...
                <div><strong>Semester:</strong> 1</div>
...
<table className="w-full mt-4 text-sm text-center border border-gray-300 rounded-lg overflow-hidden">
  ...
  <tbody className="text-gray-800">
    ...
    {/* CGPA Row */}
       ...
       <div></div> // <p>No results found for BSCS Semester 1 with seat no: {seatNo}</p>
      )}
    </div>
  );
};

export default BscsR1;
